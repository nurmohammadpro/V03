  ...
  app.get("/api/admin/metrics", async (request, reply) => {
    ...
    const [
      ...
      db
        ...
        .from(generationRuns)
        .where(sql`${generationRuns.createdAt} >= ${last7}`),
      db
        .select({ genPrev7: sql<number>`count(*)` })
        .from(generationRuns)
        .where(and(sql`${generationRuns.createdAt} >= ${prev7}`, sql`${generationRuns.createdAt} < ${last7}`)),
      db.select({ users7: sql<number>`count(*)` }).from(users).where(sql`${users.createdAt} >= ${last7}`),
      db
        .select({ usersPrev7: sql<number>`count(*)` })
        .from(users)
        .where(and(sql`${users.createdAt} >= ${prev7}`, sql`${users.createdAt} < ${last7}`)),
      db.select({ projects7: sql<number>`count(*)` }).from(projects).where(sql`${projects.createdAt} >= ${last7}`),
      db
        .select({ projectsPrev7: sql<number>`count(*)` })
        .from(projects)
        .where(and(sql`${projects.createdAt} >= ${prev7}`, sql`${projects.createdAt} < ${last7}`)),
      db
        .select({ genFailedToday: sql<number>`count(*)` })
        .from(generationRuns)
        .where(and(sql`${generationRuns.createdAt} >= ${today}`, eq(generationRuns.status, "failed"))),
    ]);
    ...
    const userRows = await db
      ...
      .from(users)
      .where(sql`${users.createdAt} >= ${trendStart}`)
